...
<body>
  ...
  if (isset($_POST['recuperarSenha'])) {
    ...
    $sql = "SELECT * FROM usuarioadmin WHERE email = '$email'";
  ...
  if (isset($_POST['alterarSenha'])) {
    ...
    if (isset($_POST['alterarSenha'])) {
      ...
      $sql = "SELECT id FROM usuarioadmin WHERE email = '$email'";
      $resultado = $conexao->query($sql);
      ...
      if ($resultado->num_rows > 0) {
        ...
        if ($confirmarId == $idEsperado) {
          ...
          $sqlUpdate = "UPDATE usuarioadmin SET senha_hash='$hashedPassword' WHERE email='$email'";
